create or replace view ae16_tab3a as
select nvl(numnuts,'0000') numnuts,nvl(nazevnumnuts,'ÈR') nazevnumnuts,
count(distinct novy_tazsoubor)/*count(distinct case when idvysl is not null then novy_tazsoubor else null end)*/ pocet_tazsoubor,
count(distinct case when idvysl is not null then novy_tazsoubor else null end)/count(distinct novy_tazsoubor) p_tazsoubor,
count(*) pocet_vyb,
sum(case when idvysl is not null then 1 else 0 end) pocet_uk,
sum(case when idvysl is not null then 1 else 0 end)/count(*) p_uk,
sum(case when (setreni='ISD' and ((idvysl=2) or (idvysl=11 and setritmodul is not null)))
               or (setreni='AES' and idvysl is not null) then 1 else 0 end) pocet_uprvyb,
sum(case when (setreni='ISD' and idvysl=11 and pocetaes>0 and setritmodul=1) OR (setreni='AES'and idvysl=11) then 1 else 0 end) pocet_vys,
case when sum(case when (setreni='ISD' and ((idvysl=2) or (idvysl=11 and setritmodul is not null)))
               or (setreni='AES' and idvysl is not null) then 1 else 0 end)=0 then 0 else sum(case when (setreni='ISD' and idvysl=11 and pocetaes>0 and setritmodul=1) OR (setreni='AES'and idvysl=11) then 1 else 0 end)/sum(case when (setreni='ISD' and ((idvysl=2) or (idvysl=11 and setritmodul is not null)))
               or (setreni='AES' and idvysl is not null) then 1 else 0 end) end p_vys_uk,
case when sum(case when (setreni='ISD' and ((idvysl=2) or (idvysl=11 and setritmodul is not null)))
               or (setreni='AES' and idvysl>=11 AND IDVYSL<40) then 1 else 0 end)=0 then 0 else sum(case when (setreni='ISD' and idvysl=11 and pocetaes>0 and setritmodul=1) OR (setreni='AES'and idvysl=11) then 1 else 0 end)/sum(case when (setreni='ISD' and ((idvysl=2) or (idvysl=11 and setritmodul is not null)))
               or (setreni='AES' and idvysl>=11 AND IDVYSL<40) then 1 else 0 end) end p_CISTA_VYS,
sum(case when setreni='ISD' and  ((idvysl=2) OR (idvysl=11 and setritmodul=3)) then 1 else 0 end) pocet_vysAES34,
case when sum(case when (setreni='ISD' and ((idvysl=2) or (idvysl=11 and setritmodul is not null)))
               or (setreni='AES' and idvysl is not null) then 1 else 0 end)=0  then 0 else sum(case when setreni='ISD' and  ((idvysl=2) OR (idvysl=11 and setritmodul=3))  then 1 else 0 end)/ sum(case when (setreni='ISD' and ((idvysl=2) or (idvysl=11 and setritmodul is not null)))
               or (setreni='AES' and idvysl is not null) then 1 else 0 end) end p_vysAES34_uk,
sum(case when (setreni='ISD' and(idvysl=11 and setritmodul=4)) or (setreni='AES' and (idvysl>11 and idvysl<40)) then 1 else 0 end) pocet_NEVYS,
case when sum(case when (setreni='ISD' and ((idvysl=2) or (idvysl=11 and setritmodul is not null)))
               or (setreni='AES' and idvysl is not null) then 1 else 0 end)=0 then 0 else sum(case when (setreni='ISD' and(idvysl=11 and setritmodul=4)) or (setreni='AES' and (idvysl>11 and idvysl<40)) then 1 else 0 end)/sum(case when (setreni='ISD' and ((idvysl=2) or (idvysl=11 and setritmodul is not null)))
               or (setreni='AES' and idvysl is not null) then 1 else 0 end) end p_nevys,
sum(case when (setreni='AES' and (idvysl>40)) then 1 else 0 end) pocet_admin,
case when sum(case when (setreni='ISD' and ((idvysl=2) or (idvysl=11 and setritmodul is not null)))
               or (setreni='AES' and idvysl is not null) then 1 else 0 end)=0 then 0 else sum(case when (setreni='AES' and (idvysl>40)) then 1 else 0 end)/sum(case when (setreni='ISD' and ((idvysl=2) or (idvysl=11 and setritmodul is not null)))
               or (setreni='AES' and idvysl is not null) then 1 else 0 end) end p_admin,
sum(case when idvysl is null then 1 else 0 end) pocet_neuk,
sum(case when idvysl is null then 1 else 0 end)/count(*) p_neuk,
sum(spravny_pocet_aes) pocet_aes,
sum(pocetaes) pocet_aes_uk,
sum(pocetaes)/sum(spravny_pocet_aes) p_aes_uk,
...
/*slouèené tabulky*/
(select * from
(select t.*,case when pocetaes_odhad is null then nvl(pocetaes,0) else pocetaes_odhad end spravny_pocet_aes from   AES_MONITORING t)
left join
(select * from CIS_KRAJ_OKRES
where numnuts like '%00' ) cis
on (substr(novy_tazsoubor,0,2) = substr(numnuts,0,2))
left join
...
having (numnuts is not null and nazevnumnuts is not null) or (numnuts is  null and nazevnumnuts is  null)
group by rollup(numnuts,nazevnumnuts);
