create or replace view ae16_tab5 as
...
sum(case when (idvysl=21)  then 1 else 0 end) odm_neochota,
case when sum(case when idvysl is not null then 1 else 0 end)=0 then 0 else sum(case when (idvysl=21) then 1 else 0 end)/sum(case when idvysl is not null then 1 else 0 end) end p_odm_neochota,
...
sum (case when idvysl=55 then 1 else 0 end) as admin_neexPanel,
case when sum(case when (idvysl is not null or idvysl<>1) then 1 else 0 end)= 0 then 0 else sum (case when idvysl=55 then 1 else 0 end)/sum (case when (idvysl is not null or idvysl<>1) then 1 else 0 end) end as "%_admin_neexPanel"
from  AES_MONITORING t where setreni='AES'
