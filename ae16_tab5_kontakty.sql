create or replace view ae16_tab5_kontakty as
...
case when count(distinct pk) = 0 then 0 else sum(case when id_akce=100 then 1 else 0 end)/ count(distinct pk) end as "prum_editace"
 from
 (select * from (
select * from ae16_kontakty_t
union
select * from ae16_kontakty_g
/*union
select * from ae16_kontakty*/)
where pk in (select to_number(idokres||idpagina) from AES_MONITORING m)
)
