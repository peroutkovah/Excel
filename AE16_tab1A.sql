create or replace view ae16_tab1a as
select nvl(numnuts,'0000') numnuts,nvl(nazevnumnuts,'ÈR') nazevnumnuts,
count(distinct novy_tazsoubor /*case when idvysl is not null then novy_tazsoubor else null end*/) pocet_tazsoubor,
case when count(distinct novy_tazsoubor)=0 then 0 else count(distinct case when idvysl is not null then novy_tazsoubor else null end)/count(distinct novy_tazsoubor) end p_tazsoubor,
count(*) pocet_vyb,
...
/*slouèené tabulky*/
(select * from
(select t.*, case when pocetaes_odhad is null then nvl(pocetaes,0) else pocetaes_odhad end spravny_pocet_aes from   AES_MONITORING t where setreni='ISD')
left join
