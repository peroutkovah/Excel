create or replace view ae16_tab2a as
...
sum(case when (idvysl=11) then 1 else 0 end) pocet_vys,
case when sum(case when idvysl is not null then 1 else 0 end)=0 then 0 else sum(case when (idvysl=11) then 1 else 0 end)/sum(case when idvysl is not null then 1 else 0 end) end p_vys_uk,
case when sum(case when idvysl>=11 and idvysl<40 then 1 else 0 end)=0 then 0 else sum(case when (idvysl=11) then 1 else 0 end)/sum(case when idvysl>=11 and idvysl<40 then 1 else 0 end) end p_CISTA_VYS,
sum(case when (idvysl>11 and idvysl<40) then 1 else 0 end) pocet_nevys,
case when sum(case when idvysl is not null then 1 else 0 end)=0 then 0 else sum(case when (idvysl>11 and idvysl<40) then 1 else 0 end)/sum(case when idvysl is not null then 1 else 0 end) end p_nevys,
sum(case when (idvysl>40) then 1 else 0 end) pocet_odpad,
case when sum(case when idvysl is not null then 1 else 0 end)=0 then 0 else sum(case when (idvysl>40) then 1 else 0 end)/sum(case when idvysl is not null then 1 else 0 end) end p_odpad,
...
/*slouèené tabulky*/
(select * from
(select  t.*,case when pocetaes_odhad is null then nvl(pocetaes,0) else pocetaes_odhad end spravny_pocet_aes from   AES_MONITORING t where setreni='AES')
left join
